...
// The following is taken from the calendar popup
...
function unhideSelectObjects(a) {
	for (var j = 0; j < document.forms.length; j++) {
		var f = document.forms[j];
		for (var i = 0; i < f.elements.length; i++) {
			if (f.elements[i].type.indexOf('select') > -1 && f.elements[i].name != 'cpMonth' && f.elements[i].name != 'oe_searchListBox') {
...
function PopupWindow_hidePopup() {
	...
	if (this.divName != null) {
		if (this.use_gebi) {
			if (document.getElementById(this.divName).style.visibility == "visible") {
				unhideSelectObjects(document.getElementById(this.divName).form);
			}
...
function CP_showCalendar(anchorname) {
	hideSelectObjects(document.getElementById(this.divName).form);
	if (arguments.length>1) {
...
//				result += '<TD CLASS="'+this.cssPrefix+'cpMonthNavigation" WIDTH="10"><A CLASS="'+this.cssPrefix+'cpMonthNavigation" HREF="'+refreshLink+'('+this.index+','+next_month+','+next_month_year+');">&gt;</A></TD>';
				...
				if (this.isShowYearNavigationInput) {
					result += '<TD CLASS="'+this.cssPrefix+'cpYearNavigation" WIDTH="36"><INPUT NAME="cpYear" CLASS="'+this.cssPrefix+'cpYearNavigation" SIZE="4" MAXLENGTH="4" VALUE="'+year+'" onFocus="this.select()"  onKeyUp="if (this.value.length == 4) { this.blur(); }" onBlur="'+refresh+'('+this.index+','+month+',this.value-0);"></TD>';
					}
				else {
...
function oe_formSubmitted(oeId) {
